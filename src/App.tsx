...
function MainApp() {
  ...
  const handleExportCSV = () => {
    ...
    const headers = [
      'ID', 'Nome', 'Custo (R$)', 'Marketplace', 'Tipo Anúncio', 'Imposto (%)', 'Ads (%)', 'Margem Alvo (%)', 'Preço de Venda (R$)', 'Lucro Líquido (R$)', 'Margem Real (%)',
      'Categoria ID', 'Fulfillment (R$)', 'Frete (R$)', 'Amazon Tier ID'
    ];
  ...
  const handleImportCSV = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = (e) => {
      ...
      for (let i = 1; i < lines.length; i++) {
        ...
        const marketplaceStr = getVal('Marketplace');
        const marketplace = (marketplaceStr === 'mercadolivre' || marketplaceStr === 'shopee' || marketplaceStr === 'amazon') 
          ? marketplaceStr as Marketplace
          : activeTab;

        const categoryId = getVal('Categoria ID') || 
                           (marketplace === 'mercadolivre' ? 'casa_moveis' : (marketplace === 'amazon' ? 'casa' : 'geral'));

        const adTypeStr = getVal('Tipo Anúncio');
        const adType: AdType = (adTypeStr === 'premium' || adTypeStr === 'classico') ? adTypeStr as AdType : 'classico';

        const importedSku: SKUItem = {
          id: getVal('ID') || Math.random().toString(36).substring(2, 11),
  ...
  const addSKU = () => {
    const newSku: SKUItem = {
      ...
      cost: simParams.cost,
      categoryId: activeTab === 'mercadolivre' ?
'casa_moveis' : (activeTab === 'amazon' ? 'casa' : 'geral'),
      adType: simParams.adType,
      adsPercent: globalAds,
      fulfillment: activeTab === 'amazon' && simParams.adType === 'premium' ?
AMAZON_FBA_TIERS[0].cost : 0,
      amazonTierId: activeTab === 'amazon' && simParams.adType === 'premium' ?
AMAZON_FBA_TIERS[0].id : undefined,
        ...
        <div className="flex items-center gap-2 md:gap-4 overflow-hidden">
          <div className="flex bg-slate-800 p-1 rounded-lg border border-slate-700 overflow-x-auto no-scrollbar max-w-[150px] sm:max-w-none">
            ...
            <button 
              ...
              className={`px-2 sm:px-4 py-1.5 text-[8px] sm:text-[10px] font-black uppercase rounded-md transition-all flex items-center gap-2 shrink-0 ${activeTab === 'shopee' ?
'bg-orange-600 text-white shadow-lg' : 'text-slate-400 hover:text-white'}`}
            >
            ...
            <button 
              ...
              className={`px-2 sm:px-4 py-1.5 text-[8px] sm:text-[10px] font-black uppercase rounded-md transition-all flex items-center gap-2 shrink-0 ${activeTab === 'amazon' ?
'bg-yellow-500 text-black shadow-lg' : 'text-slate-400 hover:text-white'}`}
            >
              <div className="w-1.5 h-1.5 sm:w-2 sm:h-2 rounded-full bg-black"></div>
...
'Mercado Livre v2025' : 
               activeTab === 'shopee' ?
'Shopee Maio 2026' : 'Amazon Brasil Full'}
            </p>
          </div>
          <button 
              ...
              <aside className="col-span-12 lg:col-span-3 flex flex-col gap-4 overflow-y-auto pr-1">
                ...
                <div className="bg-white rounded-xl border border-slate-200 p-6 shadow-sm flex flex-col gap-6">
                  <div className="flex items-center justify-between border-b border-slate-100 pb-4">
                    ...
                    <span className={`text-[9px] px-2 py-0.5 rounded-full font-bold ${activeTab === 'mercadolivre' ?
'bg-yellow-100 text-yellow-800' : activeTab === 'shopee' ? 'bg-orange-100 text-orange-700' : 'bg-yellow-50 text-yellow-700'}`}>LIVE API</span>
                  </div>
...
'focus:ring-2 focus:ring-orange-500/20 focus:border-orange-500' : 
                    ...
                    <button 
                      ...
                      className={`w-full py-3.5 font-black text-xs uppercase tracking-widest rounded-xl transition-all shadow-lg active:scale-95 flex items-center justify-center gap-2 ${
                        activeTab === 'mercadolivre' ?
'bg-yellow-400 text-black hover:bg-yellow-500 shadow-yellow-400/10' : 
                        activeTab === 'shopee' ?
'bg-orange-600 hover:bg-orange-700 text-white shadow-orange-500/20' : 
                ...
                <div className={`rounded-xl p-6 text-white shadow-xl transition-colors duration-500 ${
                  activeTab === 'mercadolivre' ?
'bg-yellow-500 text-black shadow-yellow-200' : 
                  activeTab === 'shopee' ?
...
'text-orange-600 hover:bg-orange-50' :
                    ...
                    >
                      Adicionar ao {activeTab === 'mercadolivre' ?
'ML' : activeTab === 'shopee' ? 'Shopee' : 'Amazon'}
                    </button>
                  </div>
                </div>
              </aside>
              ...
              <main className="col-span-12 lg:col-span-9 flex flex-col gap-4 min-h-0 pb-4">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-4 shrink-0">
                  <SummaryCard 
                    ...
                    accent={activeTab === 'mercadolivre' ?
'black' : activeTab === 'shopee' ? 'orange' : 'yellow'}
                  />
                  <SummaryCard 
                ...
                <section className="bg-white col-span-1 lg:col-span-9 rounded-xl border border-slate-200 shadow-sm flex flex-col min-h-[500px] lg:overflow-hidden h-full">
                  <div className="px-4 md:px-6 py-4 border-b border-slate-100 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 bg-white z-10 shrink-0">
                    <h3 className="text-xs md:text-sm font-black text-slate-800 uppercase tracking-tight flex items-center gap-2 shrink-0">
                      <LayoutDashboard className={`w-4 h-4 mr-1 ${activeTab === 'mercadolivre' ?
'text-black font-black' : 'text-orange-600'}`} />
                      <span className="truncate">Lista de SKU {activeTab === 'mercadolivre' ?
'Mercado Livre' : activeTab === 'shopee' ? 'Shopee' : 'Amazon'}</span>
                    </h3>
                      ...
                      <thead className="sticky top-0 bg-slate-50/95 backdrop-blur-sm z-20 shadow-sm">
                        <tr className="text-[10px] font-black text-slate-500 uppercase tracking-widest border-b border-slate-100">
                          ...
                          <th className="px-5 py-4 border-b border-slate-100 text-center uppercase">
                            {activeTab === 'mercadolivre' ?
'Tipo Anúncio' : 'Comissão'}
                          </th>
                          <th className="px-5 py-4 border-b border-slate-100 text-center">
                            {activeTab === 'mercadolivre' ?
'Frete (Pago por você)' : 'Custos Envio (Vendedor)'}
                          </th>
                          <th className="px-5 py-4 border-b border-slate-100 text-center uppercase">Ads & Imp. %</th>
                          <th className={`px-5 py-4 border-b border-slate-100 text-center ${activeTab === 'mercadolivre' ?
'bg-yellow-400/10' : 'bg-orange-50/50'}`}>Margem Alvo</th>
                          <th className={`px-5 py-4 border-b border-slate-100 text-center ${activeTab === 'mercadolivre' ?
'bg-yellow-400/20 text-black' : 'bg-orange-600/5 text-orange-600'}`}>Preço Venda</th>
          ...
          <Route path="/manager" element={
            <div className="col-span-12 flex items-center justify-center bg-white rounded-2xl border border-slate-200 p-20 shadow-sm h-full">
              <div className="text-center">
                ...
                <Link to="/" className={`inline-block mt-8 px-8 py-3 rounded-xl font-black text-xs uppercase tracking-widest shadow-lg ${activeTab === 'mercadolivre' ?
'bg-yellow-400 text-black' : 'bg-blue-600 text-white'}`}>Voltar ao Dashboard</Link>
              </div>
            </div>
          } />
            ...
            <div className="col-span-12 flex flex-col gap-6 h-full overflow-y-auto pb-10">
              <div className="bg-white rounded-2xl border border-slate-200 p-8 shadow-sm">
                ...
                <div className="mt-12 pt-8 border-t border-slate-100 flex justify-center">
                  <Link to="/" className={`px-10 py-4 rounded-xl font-black text-xs uppercase tracking-widest shadow-xl transition-all active:scale-95 ${activeTab === 'mercadolivre' ?
'bg-yellow-400 text-black shadow-yellow-200' : 'bg-slate-900 text-white shadow-slate-200'}`}>
...
function ProfessionallyPolishedRow({ sku, onUpdate, onRemove, onCompare }: { 
  ...
  const themeColor = isML ?
'blue' : (sku.marketplace === 'shopee' ? 'orange' : 'yellow');
      ...
      <td className="px-6 py-4">
        ...
        <div className="flex items-center gap-2 mt-1.5">
          ...
          <span className={`text-[9px] font-black uppercase ${
            isML ?
'text-yellow-600' : isAmazon ? 'text-yellow-600' : 'text-orange-500'
          }`}>
            {isML ?
`Comissão ${((sku.adType === 'classico' ? (category?.classicoRate || 0) : (category?.premiumRate || 0)) * 100).toFixed(1)}%` : 
             isAmazon ?
`Referral ${((amazonCategory?.rate || 0) * 100).toFixed(0)}%` : 
             `Padrão Shop.`}
          </span>
        </div>
      </td>
      ...
      <td className="px-5 py-4">
        <div className={`group/input relative flex items-center bg-slate-50 border rounded-lg px-2 py-1.5 transition-all shadow-inner-sm ${
          isML ?
'focus-within:border-yellow-400 border-slate-200' : 
          isAmazon ?
'focus-within:border-yellow-500 border-slate-200' :
      ...
      <td className="px-5 py-4 text-center">
        <div className="inline-flex bg-slate-100 p-1 rounded-lg border border-slate-200">
          ...
          >
            {isML ?
'Clássico' : isAmazon ? 'FBM' : 'Padrão'}
          </button>
          <button 
            onClick={() => onUpdate(sku.id, { adType: 'premium' })}
            className={`px-3 py-1 text-[9px] font-black uppercase rounded-md transition-all ${sku.adType === 'premium' ?
(isML ? 'bg-yellow-400 text-black font-black' : isAmazon ? 'bg-yellow-500 text-black' : 'bg-orange-600 text-white') + ' shadow-sm border border-transparent' : 'text-slate-500 hover:text-slate-800'}`}
          >
            {isML ?
'Premium' : isAmazon ? 'FBA' : 'Frete Gr.'}
          </button>
        </div>
      </td>
      ...
      <td className="px-5 py-4 text-center">
        <div className="flex flex-col items-center gap-1">
          <div className="relative group/shipping w-full">
            <button className={`w-full flex items-center justify-between text-xs font-bold bg-slate-50 border rounded-lg px-3 py-2 transition-all shadow-sm ${isML ?
'border-slate-200 hover:border-yellow-400' : 'border-slate-200 hover:border-orange-300'}`}>
              <span className="text-slate-400 uppercase text-[9px]">{isML ?
'Base' : 'Envio'}</span>
            ...
            <div className="absolute top-full left-0 mt-2 hidden group-hover/shipping:block z-[100] bg-white border border-slate-200 shadow-2xl rounded-xl p-3 w-56 animate-in fade-in slide-in-from-top-2 duration-200">
              <div className="text-[10px] font-black uppercase text-slate-400 mb-3 border-b border-slate-100 pb-2">
                {isML ?
'Tabela ME2 (Valor Cheio)' : isAmazon ? 'Logística Amazon' : 'Custo de Envio (Vendedor)'}
              </div>
...
(
      ...
      <td className="px-5 py-4">
        <div className="flex flex-col gap-1.5">
          <div className={`flex items-center bg-slate-50 border rounded-lg px-2 py-1 transition-all ${isML ?
'border-slate-200 focus-within:border-yellow-400' : 'border-slate-200 focus-within:border-orange-500'}`}>
...
'border-slate-200 focus-within:border-yellow-400' : 'border-slate-200 focus-within:border-orange-500'}`}>
      ...
      <td className={`px-5 py-4 ${isML ?
'bg-yellow-50/30' : 'bg-orange-50/30'}`}>
        <div className="flex flex-col items-end gap-1">
          <div className={`flex items-center bg-white border rounded-lg px-3 py-1.5 w-full shadow-sm ${isML ?
'border-yellow-200' : 'border-orange-200'}`}>
            <input 
              type="number" 
              min="0"
              max="100"
              ...
              className={`w-full text-right bg-transparent text-sm font-black outline-none ${isML ?
'text-black' : 'text-orange-600'}`}
            />
            <span className={`text-[10px] font-bold ml-1 ${isML ?
'text-slate-400' : 'text-orange-300'}`}>%</span>
          </div>
          <p className={`text-[9px] font-black uppercase tracking-wider ${isML ?
'text-slate-400' : 'text-orange-400/60'}`}>Breakeven: R$ {result.breakEvenPrice.toFixed(2)}</p>
        </div>
      </td>

      <td className={`px-5 py-4 ${isML ?
'bg-yellow-400/5' : isAmazon ? 'bg-yellow-600/5' : 'bg-orange-600/5'}`}>
        <div className="text-right">
          <p className={`text-lg font-black tracking-tight leading-none ${isML ?
'text-black' : isAmazon ? 'text-yellow-600' : 'text-orange-600'}`}>
            R$ {result.salePrice.toLocaleString('pt-BR', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
          </p>
          <div className="flex items-center justify-end gap-1 mt-1.5">
            <span className={`text-[8px] font-black uppercase px-1.5 py-0.5 rounded-full ${
              isML ?
(result.salePrice >= 79 ? 'bg-green-100 text-green-700' : 'bg-orange-100 text-orange-700') : 
              isAmazon ?
(result.commission === 1 ? 'bg-blue-100 text-blue-700' : 'bg-green-100 text-green-700') :
              'bg-orange-100 text-orange-700'
            }`}>
              {isML ?
(result.salePrice >= 79 ? 'Frete Grátis' : 'Taxa Fixa R$6') : 
               isAmazon ?
(result.commission === 1 ? 'Comissão Mínima' : 'Venda Amazon') :
...
'bg-red-100 text-red-700 shadow-sm shadow-red-200' : 
                result.realMarginPercent < 5 ?
'bg-amber-100 text-amber-700 border border-amber-200' : 
                'bg-green-100 text-green-700'
              }`}>
                {result.realMarginPercent >= 0 ?
'+' : ''}{result.realMarginPercent.toFixed(1)}%
              ...
              <div className="absolute right-0 top-full mt-2 hidden group-hover/breakdown:block z-[110] bg-white border border-slate-200 shadow-2xl rounded-xl p-4 w-72 animate-in fade-in slide-in-from-top-2 duration-200 text-left">
                <div className="flex items-center justify-between mb-3 border-b border-slate-100 pb-2">
                  ...
                  <span className={`text-[9px] font-bold px-2 py-0.5 rounded-full ${isML ?
'text-black bg-yellow-400' : isAmazon ? 'text-yellow-600 bg-yellow-50' : 'text-orange-600 bg-orange-50' }`}>PV: R$ {result.salePrice.toLocaleString('pt-BR', { minimumFractionDigits: 2 })}</span>
                </div>
                ...
                <div className="space-y-3">
                  ...
                  <BreakdownItem 
                    label={isML ?
"Comissão ML" : isAmazon ? "Referral Amazon" : "Comissão Shopee"} 
                    value={result.components.commissionAbs} 
                    percent={(result.components.commissionAbs / result.salePrice) * 100} 
                    color="orange" 
                    rateLabel={isML ?
`${((sku.adType === 'classico' ? (category?.classicoRate || 0) : (category?.premiumRate || 0)) * 100).toFixed(1)}% + R$${result.fixedFee}` : isAmazon ?
`${((amazonCategory?.rate || 0) * 100).toFixed(0)}%` : `${sku.adType === 'premium' ?
'20%' : '14%'} + R$4`}
                  />
                  <BreakdownItem 
                    label={isAmazon ?
"FBA / Envio" : "Frete (Líquido)"} 
                    ...
                    color="blue" 
                    rateLabel={isML ?
"60% Desc." : isAmazon ? "Fulfillment" : "Valor Fixo"}
                  />
                  <BreakdownItem 
                    label={isML ?
"Ads (Publicidade)" : isAmazon ? "Amazon Ads" : "Shopee Ads"} 
...
value={result.components.taxesAbs} 
                  ...
                  <div className="pt-3 mt-1 border-t border-slate-100 flex justify-between items-center">
                    ...
                    <span className={`text-sm font-black p-1.5 rounded-lg ${isProfitable ?
'text-green-600 bg-green-50' : 'text-red-500 bg-red-50' }`}>
        ...
        <div className="flex items-center gap-1">
          <button 
            ...
            className={`p-2 rounded-xl transition-all active:scale-90 ${
              isML ?
'hover:bg-yellow-100 text-slate-400 hover:text-black' : 
              isAmazon ?
'hover:bg-yellow-50 text-yellow-600/30 hover:text-yellow-600' :
      ...
      <div className="space-y-2 md:space-y-3">
        ...
        <div className="grid grid-cols-2 gap-2">
          <div className="bg-white/60 p-2 rounded-lg border border-white">
            ...
            <p className={`text-[10px] md:text-xs font-black ${result.netProfit > 0 ?
'text-green-600' : 'text-red-500'}`}>R$ {result.netProfit.toFixed(2)}</p>
          </div>
          <div className="bg-white/60 p-2 rounded-lg border border-white">
            <p className="text-[7px] md:text-[8px] font-black text-slate-400 uppercase">Margem Real</p>
            <p className={`text-[10px] md:text-xs font-black ${result.realMarginPercent > 0 ?
'text-green-600' : 'text-red-500'}`}>{result.realMarginPercent.toFixed(1)}%</p>
          </div>
        </div>
      ...
      >
        ...
        <div className="p-6 space-y-6">
          <div className="p-4 bg-blue-50 border border-blue-100 rounded-xl flex gap-3">
            ...
            <p className="text-xs text-blue-700 font-medium">Esta ação atualizará a Margem Alvo de todos os produtos atualmente visíveis na lista.</p>
          </div>
