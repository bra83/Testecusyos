...
function MainApp() {
  ...
  const handleExportCSV = () => {
    ...
    const rows = filteredSkus.map(sku => {
      // CORREÇÃO: Pegando a categoria correta baseado no marketplace
      const category = sku.marketplace === 'amazon'
        ? (AMAZON_CATEGORIES.find(c => c.id === sku.categoryId) || AMAZON_CATEGORIES[0])
        : (CATEGORIES.find(c => c.id === sku.categoryId) || CATEGORIES[0]);
  ...
  const stats = useMemo(() => {
    ...
    const calculated = marketplaceSkus.map(s => {
      // CORREÇÃO: Usar objeto correto por aba
      const category = activeTab === 'amazon'
        ? (AMAZON_CATEGORIES.find(c => c.id === s.categoryId) || AMAZON_CATEGORIES[0])
        : (CATEGORIES.find(c => c.id === s.categoryId) || CATEGORIES[0]);
...
function ProfessionallyPolishedRow({ sku, onUpdate, onRemove, onCompare }: { 
  ...
  // CORREÇÃO: Passar a categoria correta baseada no marketplace ativo para o Row
            ...
            <div className="relative group/breakdown cursor-help">
              ...
              <div className="absolute right-0 top-full mt-2 hidden group-hover/breakdown:block z-[110] bg-white border border-slate-200 shadow-2xl rounded-xl p-4 w-72 animate-in fade-in slide-in-from-top-2 duration-200 text-left">
                ...
                <div className="space-y-3">
                  ...
                  />
                  <BreakdownItem 
                    label="Impostos Est." value={result.components.taxesAbs} 
...
function SimulationPreview({ cost, margin, adType, shipping, globalTax, globalAds }: { 
  ...
  // CORREÇÃO: Enviando category nos 3 casos
