...
// IMPORTANTE: Alterado de 'motion/react' para 'framer-motion' para compatibilidade com a maioria das versões instaladas.
...
// Evitam tela branca caso alguma constante não seja importada corretamente do constants.ts
const SAFE_CATEGORIES = Array.isArray(CATEGORIES) && CATEGORIES.length > 0 
  ? CATEGORIES 
  : [{ id: 'default', name: 'Geral', classicoRate: 0.13, premiumRate: 0.18 }] as any[];

const SAFE_AMAZON_CATEGORIES = Array.isArray(AMAZON_CATEGORIES) && AMAZON_CATEGORIES.length > 0 
  ? AMAZON_CATEGORIES 
  : [{ id: 'default', name: 'Geral', rate: 0.15 }] as any[];

const SAFE_FBA_TIERS = Array.isArray(AMAZON_FBA_TIERS) && AMAZON_FBA_TIERS.length > 0 
  ? AMAZON_FBA_TIERS 
  : [{ id: 'pequeno', name: 'Pequeno', cost: 10 }] as any[];

const SAFE_SHIPPING_RATES = Array.isArray(SHIPPING_RATES) ? SHIPPING_RATES : [];
...
function MainApp() {
  ...
  const handleExportCSV = () => {
    ...
    const rows = filteredSkus.map(sku => {
      const category = sku.marketplace === 'amazon' 
        ? (SAFE_AMAZON_CATEGORIES.find(c => c.id === sku.categoryId) || SAFE_AMAZON_CATEGORIES[0])
        : (SAFE_CATEGORIES.find(c => c.id === sku.categoryId) || SAFE_CATEGORIES[0]);
  ...
  const addSKU = () => {
    const newSku: SKUItem = {
      ...
      fulfillment: activeTab === 'amazon' && simParams.adType === 'premium' ? SAFE_FBA_TIERS[0].cost : 0,
      amazonTierId: activeTab === 'amazon' && simParams.adType === 'premium' ? SAFE_FBA_TIERS[0].id : undefined,
  ...
  const stats = useMemo(() => {
    ...
    const calculated = marketplaceSkus.map(s => {
      const category = s.marketplace === 'amazon' 
        ? (SAFE_AMAZON_CATEGORIES.find(c => c.id === s.categoryId) || SAFE_AMAZON_CATEGORIES[0])
        : (SAFE_CATEGORIES.find(c => c.id === s.categoryId) || SAFE_CATEGORIES[0]);
...
function ProfessionallyPolishedRow({ sku, onUpdate, onRemove, onCompare }: { 
  ...
  const category = useMemo(() => SAFE_CATEGORIES.find(c => c.id === sku.categoryId) || SAFE_CATEGORIES[0], [sku.categoryId]);
  const amazonCategory = useMemo(() => SAFE_AMAZON_CATEGORIES.find(c => c.id === sku.categoryId) || SAFE_AMAZON_CATEGORIES[0], [sku.categoryId]);
  
  const result = useMemo(() => {
    const activeCategory = sku.marketplace === 'amazon' ? amazonCategory : category;
    return calculatePrice({ ...sku, category: activeCategory, marketplace: sku.marketplace });
  }, [sku, category, amazonCategory]);
      ...
      <td className="px-6 py-4">
        ...
        <div className="flex items-center gap-2 mt-1.5">
          {isML && (
            ...
            >
              {SAFE_CATEGORIES.map((c: any) => <option key={c.id} value={c.id} className="text-sm font-sans">{c.name}</option>)}
            </select>
          )}
          {isAmazon && (
            <select 
              ...
              className="text-[9px] font-bold text-slate-400 uppercase cursor-pointer hover:text-yellow-600 bg-transparent outline-none"
            >
              {SAFE_AMAZON_CATEGORIES.map((c: any) => <option key={c.id} value={c.id} className="text-sm font-sans">{c.name}</option>)}
            </select>
          )}
          ...
          <div className="relative group/shipping w-full">
            ...
            <div className="absolute top-full left-0 mt-2 hidden group-hover/shipping:block z-[100] bg-white border border-slate-200 shadow-2xl rounded-xl p-3 w-56 animate-in fade-in slide-in-from-top-2 duration-200">
              ...
              <div className="space-y-1.5 max-h-56 overflow-y-auto custom-scrollbar pr-1 text-left">
                {isML ? (
                  SAFE_SHIPPING_RATES.map((rate: any) => (
                    <button 
                      key={rate.weight}
...
function SimulationPreview({ cost, margin, adType, shipping, globalTax, globalAds }: { 
  ...
  const mlCategory = SAFE_CATEGORIES.find(c => c.id === 'casa_moveis') || SAFE_CATEGORIES[0];
  ...
  const shopeeCategory = SAFE_CATEGORIES.find(c => c.id === 'geral') || SAFE_CATEGORIES[0];
  ...
  const amazonCategory = SAFE_AMAZON_CATEGORIES.find(c => c.id === 'casa') || SAFE_AMAZON_CATEGORIES[0];
  const amazonResult = calculatePrice({
    ...
    adType: adType,
    adsPercent: globalAds,
    fulfillment: adType === 'premium' ? SAFE_FBA_TIERS[0].cost : 0,
    taxPercent: globalTax,
    targetMarginPercent: margin,
